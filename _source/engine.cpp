...
Step Engine::getStep()
{
    unsigned char cachedTokenFound = blink();
    if (cachedTokenFound != 0xFF)
    {
        Step ret;
        const bool valid = makeStep(cachedTokenFound, ret, invertedSearch());
        return ret;
    }
    Step bestFound;
    ...
    {
        getUsableSteps(); _lastDepth = _level;
